(function () {
  ...
  function initForm() {
    ...
    window.addEventListener('message', async event => {
      ...
      if (dataSource == 'config') {
        ...
        if (polylines != '') {
          const decodedPath =
            google.maps.geometry.encoding.decodePath(polylines);
          ...
          const { PinView, AdvancedMarkerView } =
        ...
        const { dataSource, status, price, route, trip } = JSON.parse(
          event.data
        );
  ...
  function handleTranslation(language, translations) {
    ...
    document.getElementById('baggage').innerHTML =
      translations.TABS.TRIP.BAGGAGE;
  ...
  function typeHandler(event) {
    ...
    document.getElementById('route-form').elements['type'].value =
      selectedTypeId;
